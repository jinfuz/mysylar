...
size_t HttpRequestParser::execute(char *data, size_t len) {
    ...
    if (m_parser.upgrade) {
        //处理新协议，暂时不处理
        //HTTP upgrade header是在HTTP1.1中引入的一个HTTP头。当客户端觉得需要升级HTTP
        //协议的时候，会向服务器端发送一个升级请求，服务器端会做出相应的响应。
        //对于websocket来说，客户端在和服务器端建立连接之后，会首先发送给服务器端 
        //Upgrade: WebSocket 和 Connection: Upgrade 头。服务器端接收到客户端的请求之后，
        //如果支持webSocket协议，那么会返回同样的Upgrade: WebSocket和Connection: 
        //Upgrade 头到客户端。客户端接收到服务器端的响应之后，就知道服务器端支持websocket
        //协议了，然后就可以使用WebSocket协议发送消息了。
        ...
        if (nparsed < len) {
            //从 data尾部复制 len - nparsed 个字符到data
